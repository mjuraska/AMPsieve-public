# Purpose: Primary endpoint analysis; estimation of hazard ratio-based PE by SAP-specified sequence features; 
...
# Input:   AMP adjudicated primary endpoints, neutralization-associated sequence features calculated by Craig Magaret
...
variant <- c("mf", "ms", "ls")
dist <- c(outer(c("length.gp120.", "length.v1v2.", "length.v5.", "num.pngs.gp120.", "num.pngs.v1v2."), variant, FUN=paste0))
...
markType <- c("LengthGp120", "LengthV1V2", "LengthV5", "NumPNGSgp120", "NumPNGSv1v2")
markFileString <- c(outer(markType, variant, FUN=paste0))
  ...
  for (dose in 1:length(doseFileString)){
    ...
    for (mark in 1:length(dist)){
      ...
      if (dose==1){
        ...
        pVal[pVal$trial==trialFileString[trial] & pVal$variant==markVariant, colName] <- sfit$pWald.HRunity.2sided
        
        colName <- paste0("HRby", markType1, "Constant")
        # 2-sided Wald test of {H0: PE(v) constant for all v}
        pVal[pVal$trial==trialFileString[trial] & pVal$variant==markVariant, colName] <- sfit$pWald.HRconstant.2sided  
      }

      #xLab <- bquote(paste("IC", .(substr(markFileString[mark], start=3, stop=4)), " (", mu, "g/mL)"))
      xLab <- switch(distType, 
                     "length.gp120"="Length of gp120",
                     "length.v1v2"="Length of V1V2",
                     "length.v5"="Length of V5",
                     "num.pngs.gp120"="Number of PNGS Motifs in gp120",
                     "num.pngs.v1v2"="Number of PNGS Motifs in V1V2")
      ...
      if (dose==1){
        pval.filename <- paste0(trialFileString[trial], "_WestfallYoungAdjPvalues_neutAssocSeqFeatures_", markVariant, ".csv")
        pval.filepath <- file.path(tabDir, pval.filename)
        if (file.exists(pval.filepath)){
          p.df <- read_csv(pval.filepath, show_col_types=FALSE)
          p <- as.numeric(p.df[p.df$mark==dist[mark], -1])
      ...
      p <- ggplot(sfit, 
                  ...
                  ciLineSize=1.4,
                  pointSize=2.1,
                  bottomPlotMargin=c(-0.5, 0.7, 0, 0),
                  topPlotMargin=c(0, 0.7, -0.12, 1.43),
...
save(pVal, file=file.path(tabDir, "704_703_704and703_pValues_neutAssocSeqFeatures.RData"))
