# Purpose: Primary endpoint analysis; estimation of hazard ratio-based PE by epitope distance; 
...
# Input:   AMP adjudicated primary endpoints, epitope distances calculated by Morgane Rolland's team
...
dist <- grep("epitope.dist", colnames(dataPooled), value=TRUE)
...
pVal <- cbind(pVal, as.data.frame(matrix(NA, nrow=9, ncol=2 * 3)))
colnames(pVal)[-(1:2)] <- c(paste0("HRbyEpitopeDist", paste0("Subtype", c("B", "C", "Any")), "Unity"),
                            paste0("HRbyEpitopeDist", paste0("Subtype", c("B", "C", "Any")), "Constant"))
...
for (trial in 1:length(datFile)){
  ...
  for (dose in 1:length(doseFileString)){
    ...
    for (mark in 1:length(dist)){
      ...
      refSubtype <- substring(distType, first=nchar(distType))
      refSubtype <- switch(refSubtype, "b"="B", "c"="C", "y"="Any")
      ...
      sfitFile <- paste0(trialFileString[trial], "_sievePH_PEbyEpitopeDist_refSubtype", refSubtype, "_", markVariant, "_MITT_", doseFileString[dose], "_placebo.RData")
      save(sfit, file=file.path(outDir, sfitFile))
      
      # compute p-values only for the comparison of pooled VRC01 vs. placebo
      if (dose==1){
        
        colName <- paste0("HRbyEpitopeDistSubtype", refSubtype, "Unity")
        # 1-sided Wald test of {H0: PE(v)=0 for all v}
        pVal[pVal$trial==trialFileString[trial] & pVal$variant==markVariant, colName] <- sfit$pWtWald.HRunity.1sided
        
        colName <- paste0("HRbyEpitopeDistSubtype", refSubtype, "Constant")
      ...
      xLab <- switch(distType, 
                     "epitope.dist.b"="VRC01 Epitope Distance to\nSubtype B Reference",
                     "epitope.dist.c"="VRC01 Epitope Distance to\nSubtype C Reference",
                     "epitope.dist.any"="VRC01 Epitope Distance to\n Subtype-Agnostic Reference")
      ...
      if (dose==1){
        # if adjusted p-values were calculated for a given combination of trial and reference sequence
        if ((trial==1 && distType=="epitope.dist.b") || (trial==2 && distType=="epitope.dist.c") || (trial==3 && distType=="epitope.dist.any")){
        ...
        } else {  # if only the unadjusted p-value is reported
          p <- sfit$pWald.HRconstant.1sided
          fmt.p <- ifelse(p<0.001, "< 0.001", paste0("= ", format(p, digits=2, nsmall=2)))
          subtitle <- paste0("One-Sided Unadjusted Sieve P ", fmt.p)
        }
      ...
      # pdf(file.path(figDir, paste0(trialFileString[trial], "_sievePH_PEbyEpitopeDist_refSubtype", refSubtype, "_", markVariant, "_MITT_", doseFileString[dose], "_placebo.pdf")), 
      ...
      ggsave(file.path(figDir, paste0(trialFileString[trial], "_sievePH_PEbyEpitopeDist_refSubtype", refSubtype, "_", markVariant, "_MITT_", doseFileString[dose], "_placebo.pdf")),
...
save(pVal, file=file.path(tabDir, "704_703_704and703_pValues_epitopeDist.RData"))
