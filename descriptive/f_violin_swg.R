...
# load data
dat1 <- read.csv("../adata/v704_survival_wk80_tau_sieve_r4p_v2_cam.csv")
dat2 <- read.csv("../tables/DVE/HVTN704_site_scanning_results_DVE.csv")

colnames(dat1)
colnames(dat2)

dat1 %>% group_by(hiv1event, nisolates, hxb2.230.D.mf) %>% tally()
...
datplot$facetvar2 <- with(datplot, case_when(hxb2.230.D.mf==1 ~ paste0("D230 Match"),
                                            hxb2.230.D.mf==0 ~ paste0("D230 Mismatch")))
...
for (i in 1:length(plots)){
  pdf(paste0("../figures/descriptive/v704_violin_d230_gmt80mf", ".pdf"), width=8, height=6)
...
for (i in 1:length(plots)){
  pdf(paste0("../figures/descriptive/v704_violin_d230_gmt80mf_v2", ".pdf"), width=8, height=6)
