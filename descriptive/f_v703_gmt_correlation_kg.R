...
#
#          This program plots HVTN 703 data, adjusting for treatment arm.
#    
# Location: /trials/vaccine/p703/analysis/efficacy/code
#
# Input:
#   ../adata/v703_survival_wk80_tau_neut.csv
#
# Output:
#   ../figures/adhoc/v703_gmt_correlation.pdf
#
# Code History
# ---------------------------------------------------------------------------------------
# Date        Programmer        Details 
# ---------------------------------------------------------------------------------------
# 30Jun2020   Erika Rudnicki    Version 1.0, copied from 704
...
if(T){#Extra Functions
  ...
  draw.plot=function(dat,vars,covars,labels,lgnd,title){  
    ...
    type.matrix <- matrix(1, nrow=ncol(df), ncol=ncol(df))
    type.matrix[4:9,1:3] <- 2
    
    pairs2(df, gap = 0, lower.panel1 = panel.diag, lower.panel2 = panel.smooth, type.matrix = type.matrix,
...
# load data
dat <- read.csv("../adata/amp_sieve_v703_marks_final_v5.csv")
colnames_pairs <- c(
  "parscore1.mf", "parscore1.ms", "parscore1.ls",
  "parscore2.mf", "parscore2.ms", "parscore2.ls",
  "gmt80mf", "gmt80ms", "gmt80ls"
)

# datplot <- subset(dat, nisolates > 0)
# datplot[5,11:16]
datplot <- dat[,c('tx', colnames_pairs)]
...
pdf(paste0('../figures/descriptive/v703_gmt_correlation_kg.pdf'),width=11,height=8.5)

draw.plot(dat=datplot,
          covars=c("tx"),
          vars=c("parscore1.mf","parscore1.ms", "parscore1.ls", "parscore2.mf","parscore2.ms", "parscore2.ls", "gmt80mf", "gmt80ms", "gmt80ls"),
          labels=c("Logit Pred Prob\nIC80 > 1 mcg/ml\nMost Frequent", "Logit Pred Prob\nIC80 > 1 mcg/ml\nMost Sensitive", "Logit Pred Prob\nIC80 > 1 mcg/ml\nMost Resistant", 
                   "Log10 Pred IC80\nMost Frequent", "Log10 Pred IC80\nMost Sensitive", "Log10 Pred IC80\nMost Resistant",  
                   "Log10 IC80 \nMost Frequent", "Log10 IC80 \nMost Sensitive", "Log10 IC80 \nMost Resistant"),
          lgnd=c("Predicted Prob\nIC80 > 1 mcg/ml", "Predicted IC80 (mcg/ml)", "Observed IC80 (mcg/ml)"),
          title="HVTN 703/HPTN 081 Primary Cases with Breakthrough Neutralization Data")
