#----------------------------------------------------------------------------------------
# PROGRAM: f_cuminc_cmprsk_ampPooled.R
#
...
#
# OUTPUT: 
#     ../figures/cmprsk/v704and703_cuminc_cmprsk.pdf
#
# MAINTENANCE HISTORY:
...
adataDir <- "../adata"
adataFile1 <- "amp_sieve_pooled_marks_final_v5.csv"
adataFile2 <- c("v703and704_cmprisk_cir_mf_strata_25102022.csv", "v703and704_cmprisk_cir_ms_strata_25102022.csv", "v703and704_cmprisk_cir_ls_strata_25102022.csv")

# output directory and file names
pdfDir <- "../figures/cmprsk"
# pdfFile <- c("v703and704_cuminc_cmprsk_mf_20092022.pdf", "v703and704_cuminc_cmprsk_ms_20092022.pdf", "v703and704_cuminc_cmprsk_ls_20092022.pdf")
...
for (i in 1:length(adataFile2)) {
  ...
  cuminc_v <-
    cir %>%
    group_by(eventType, strata) %>%
    mutate(time = if_else(time==max(time), tau, time),
           level = cmpLvl,
           cuminc = cuminc.cmp) %>%
    select(strata, eventType, time, level, cuminc)
