...
sm <- read.csv("../../fsr/closed/adata/subject_master.csv")

df <- read.csv("../adata/v704_survival_wk80_tau_sieve_r4p.csv")
...
blu <- surv %>% 
  mutate(
    ptid1 = gsub("-", "", ptid, fixed = T)
  ) %>% 
  left_join(vlfp, by = c("ptid1" = "ptid")) %>% 
  mutate(
    ltfu = ifelse(s1wk80dt == "" & s1wk80dttgt == "" & s4negpostwk80fl == 0, 1, 0),
    postwk80fl = ifelse(efficacy_flag == 1 & statuswk80 == 0 & ltfu == 0, 1, 0),
    ...
    origin_time = case_when(
      s1wk80dt != "" & ltfu == 0 ~ as.character(.$s1wk80dt),
      s1wk80dt == "" & ltfu == 0 ~ as.character(.$s1wk80dttgt),
      s4negpostwk80fl == 1 ~ as.character(.$s4wk80dt)
    ),
...
# Join the new columns to the existing data source
df_new <- df %>% 
  left_join(blu) 
