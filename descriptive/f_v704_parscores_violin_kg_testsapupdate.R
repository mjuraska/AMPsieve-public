...
# change the shape of the points so I can see the new ones
dat <- subset(dat, !(hiv1event==1 & is.na(parscore1.mf) & !is.na(gmt80mf)))

dat$shape <- ifelse(dat$pub_id %in% subset(dat, hiv1event==1 & !is.na(parscore1.mf) & is.na(gmt80mf))$pub_id, 17,19)
...
datplot <- datplot %>% 
  select(tx2, contains('pred.prob.res'), contains('pred.ic80'), contains('parscore'), shape)
...
myplot <- function(dat, x, y,
  ...
  ggplot(data=dat, aes_string(x=x, y=y, color=x)) +
    ...
    geom_point(size=3, position = position_jitter(w = 0.3, h = 0), aes(shape = shape)) +
    # ylim(ylim) + 
...
plots_ylim=list('prob' = c(0.3, 0.95), 'ic80' = c(0.5, 100))
...
for (i in 1:length(plots)){
  ...
  pdf(paste0("../figures/descriptive/v704violin", plots2[i], "_20092022_kgtest.pdf"), width=8, height=8)
