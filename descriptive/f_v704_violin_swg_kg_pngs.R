...
dat1 <- read.csv("../adata/amp_sieve_v704_marks_final_v4a.csv") %>% 
  # D230 with the dichotomized number of PNGSs in V5 (<=1 vs. >1).
  # The non-dichotomized variable in the data (version 4a) is called 'num.pngs.v5.mf'.
  mutate(
    num.pngs.v5.mf.ind = ifelse(num.pngs.v5.mf > 1, 1, 0)
  )
...
dat1 %>% group_by(hiv1event, nisolates, num.pngs.v5.mf.ind) %>% tally()
...
datplot$facetvar <- with(datplot, case_when(num.pngs.v5.mf.ind==1 ~ paste0("No. of PNGSs in v5 > 1 \n", 'PE = -26 (-130, 31)'),
                                            num.pngs.v5.mf.ind==0 ~ paste0("No. of PNGSs in v5 <= 1 \n", 'PE = 52 (14, 73)')))

datplot$facetvar2 <- with(datplot, case_when(num.pngs.v5.mf.ind==1 ~ paste0("No. of PNGSs in v5 > 1"),
                                             num.pngs.v5.mf.ind==0 ~ paste0("No. of PNGSs in v5 <= 1")))

datplot %>% group_by(facetvar, facetvar2, tx2) %>% tally()

datplot$facetvar <- factor(
  datplot$facetvar, 
  levels = c(
    "No. of PNGSs in v5 <= 1 \nPE = 52 (14, 73)",
    "No. of PNGSs in v5 > 1 \nPE = -26 (-130, 31)"
  ...
  labels = c(
    expression(atop("No."~"of"~"PNGSs"~"in"~"v5"~""<="1","PE = 52 (14, 73)")),
    expression(atop("No. of PNGSs in v5 > 1","PE = -26 (-130, 31)"))
  )
)

# summarize data
datplot$gmt80mfn = as.numeric(gsub(">", "", datplot$gmt80mf))
range(datplot$gmt80mfn)
...
myplot <- function(dat, x, y, facetvar,
  ...
  ggplot(data=dat, aes_string(x=x, y=y, color=x)) +
    facet_grid(~facetvar, labeller = label_parsed) +
    geom_violin() +
...
for (i in 1:length(plots)){
  pdf(paste0("../figures/descriptive/v704_violin_pngs_gmt80mf", "05OCT2022.pdf"), width=8, height=6)
...
for (i in 1:length(plots)){
  pdf(paste0("../figures/descriptive/v704_violin_pngs_gmt80mf_v2", "05OCT2022.pdf"), width=8, height=6)
