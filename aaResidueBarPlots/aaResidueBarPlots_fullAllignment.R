# Purpose: Plotting of AA residue bar plots using all measured sequences included in the REN AA sequence alignment
...
seq_path <- paste0("o:/VTN/VTN703_704/sieve/dat/seq/work/aa/genotypic_sieve/hvtn", c(704, 703), "/env/obs")
...
for (trial in 1:length(seq_path)){
  # load in the data
  seqData <- gatherSequences(seq_path[trial])
  ...
  multiPanelPlot <- annotate_figure(multiPanelPlot, bottom=text_grob("Primary Endpoint Cases", size=14), left=text_grob("Proportion of Sequences", rot=90, size=14))
  ...
  ggpubr::ggexport(multiPanelPlot, filename=file.path(figDir, paste0(substring(protocol[trial], first=6), "_AAresidueBarPlot_fullAlignment_neutAssocPos.pdf")), width=8.5*0.9, height=11*0.9)
}
