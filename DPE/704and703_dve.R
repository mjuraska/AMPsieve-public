...
for( .feature in features.to.analyze ){
  ...
  ### Run DVE
  .dve.results <- dve3( master$hiv1fpday, master$hiv1event, .x, I( master$tx_pool == "T1+T2" ), master$southAmerica )
...
save( dve_results, file = file.path( tablesOutDir, "HVTN704and703_DVE_results_by_feature.Rda" )) 

### Forest plot
pdf( file = file.path( figuresOutDir, "HVTN704and703_DVE_results_by_feature_forest_plot.pdf" ), width = 6, height = 6)
