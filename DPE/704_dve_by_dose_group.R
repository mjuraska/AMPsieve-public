...
master <-  read.csv( "../../adata/v704_survival_wk80_tau_sieve_v5_cam.csv" ); 
...
for( .excluded.dose.group in c("T1", "T2")){
  ...
  pdf( file = file.path( figuresOutDir, paste0("HVTN704_DVE_results_by_feature_forest_plot_",tolower(.excluded.dose.group),"_excluded.pdf") ), width = 5.5, height = 6)
...
save( dve_results, file = file.path( tablesOutDir, "HVTN704_DVE_results_by_feature_and_dose_group.Rda" )) 
