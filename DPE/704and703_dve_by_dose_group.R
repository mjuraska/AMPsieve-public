...
figuresOutDir <- "/Volumes/trials/vaccine/p704/analysis/sieve/figures/DVE"
tablesOutDir <- "/Volumes/trials/vaccine/p704/analysis/sieve/tables/DVE"

options( width = 150, stringsAsFactors = FALSE );
setwd( "/Volumes/trials/vaccine/p704/analysis/sieve/code/DVE")
library( "survival" );
...
# If file does not exist, create it using Michal's script. 
if( ! file.exists("../../adata/v704and703_survival_wk80_tau_sieve_v5_cam.csv")) source("../createTrialPooledData.R")
# Read file
master <- read.csv("../../adata/v704and703_survival_wk80_tau_sieve_v5_cam.csv")
...
# Begin outer for loop for exclusion of treatment groups
for( .excluded.dose.group in c("T1", "T2")){
  ...
  # Begin inner for loop for features
  for( .feature in features.to.analyze ){
    
    .results.row.i <- .results.row.i + 1;
    
    # Be verbose:
    cat( "Computing results for feature", results[ .results.row.i, 1 ], "excluding treatment", .excluded.dose.group, "\n" );
    
    # Grab the column containing feature values from the master table
    .x <- master[ , .feature]
    
    ### NOTE now we create a copy of the master data set excluding the indicated dose group
    .master.excluded <- master[master$tx != .excluded.dose.group, ]
    .x <- .x[master$tx != .excluded.dose.group]
    
    ### Run DVE on subset of .master.excluded data excluding the indicated dosage group 
    .dve.results <- dve3( flrtime = .master.excluded$hiv1fpday, flrstatus = .master.excluded$hiv1event, flrtype = .x, Vx = I( .master.excluded$tx_pool == "T1+T2" ), str = .master.excluded$southAmerica )
    ...
    .n.events.type.1.trt <- sum( .x == 1 & .master.excluded$tx_pool == "T1+T2" & .master.excluded$hiv1event, na.rm = T)
    .n.events.type.0.trt <- sum( .x == 0 & .master.excluded$tx_pool == "T1+T2" & .master.excluded$hiv1event, na.rm = T)
    .n.events.type.1.control <- sum( .x == 1 & .master.excluded$tx_pool != "T1+T2" & .master.excluded$hiv1event, na.rm = T)
    .n.events.type.0.control <- sum( .x == 0 & .master.excluded$tx_pool != "T1+T2" & .master.excluded$hiv1event, na.rm = T)
  ...
  ### Because of a few of the results_r4p having a 0-event category, I'm setting all estimated VE's less than -1e8 to simply be -Inf
  results$VE.vs.Match.estimate[ results$VE.vs.Match.estimate < -10^8 ] <- - Inf
  results$VE.vs.Mismatch.estimate[ results$VE.vs.Mismatch.estimate < -10^8 ] <- - Inf
  
  dve_results[[ paste(.excluded.dose.group, "excluded")]] <- results
  
  cat("Drawing forest plot with treatment group", .excluded.dose.group, "excluded. \n")
  
  ### Forest plot for high dose vs control (treatment group T1 excluded)
  pdf( file = file.path( figuresOutDir, paste0("HVTN704and703_DVE_results_by_feature_forest_plot_",tolower(.excluded.dose.group),"_excluded.pdf") ), width = 5.5, height = 6)
  
  ### A few utility functions for the forest plots 
  ...
  plot( 0, 0, xaxt = "n", yaxt= "n", xlab= "", ylab= '', bty = 'n', type='n', xlim = c(-0.7,9.4), ylim = c(-1.8,nrow(results)*1.5+0.8)) ## Empty plot
  text( c(0,1,2,3,4,5.25), nrow(results)*1.5+0, c("Feature","Value","VRC01","Control","PE (%)","95% CI"),cex=0.5,adj=0.5,pos=3) # Column header row 1
  ...
  lines( c(7.5, 10.5) - 1, rep(-1.33, 2)) 
  text( c(7.5, 8, 8.5, 9, 9.5, 10, 10.5 )- 1, rep( -1.5, 7), rep( "|", 7), cex = 0.3 )
  text( c(7.5, 8, 8.5, 9, 9.5, 10, 10.5 )- 1, rep( -2, 7), c(-50,-25,0,25,50,75,100), cex = 0.5) 
  text( 8,-2.8, "PE (%)", cex =0.5)
  ...
  lines( rep(7.5, 2), c(-1, nrow(results)*1.5+0), lty = 2, col = "gray")
  ...
  points( 7.5 + 0.02*point.estimates , rep( 1.5*nrow(results):1, 2) + rep( c(0.33,-0.33), each = nrow(results) ) - 1.5, col = rep( c("red","blue"), each = nrow(results)), cex = 0.5, pch = ifelse( point.estimates < -60, NA, 19 ) )
  ...
  arrows( 7.5 + 0.02 * pmax( -60, point.estimates ), rep( 1.5*nrow(results):1, 2) + rep( c(0.33,-0.33), each = nrow(results) ) - 1.5 , 7.5 + 0.02 *  pmin( 100, upper.bounds ), rep( 1.5*nrow(results):1, 2) + rep( c(0.33,-0.33), each = nrow(results) ) - 1.5, col = rep( c("red","blue"), each = nrow(results)), code = 2, cex = 0.5, length = 0.02, angle = 90 )
...
save( dve_results, file = file.path( tablesOutDir, "HVTN704and703_DVE_results_by_feature_and_dose_group.Rda" )) 
