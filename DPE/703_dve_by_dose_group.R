...
for( .excluded.dose.group in c("T1", "T2")){
  ...
  for( .feature in features.to.analyze ){
    ...
    .dve.results <- dve3( flrtime = .master.excluded$hiv1fpday, flrstatus = .master.excluded$hiv1event, flrtype = .x, Vx = I( .master.excluded$tx_pool == "T1+T2" ) )
  ...
  pdf( file = file.path( figuresOutDir, paste0("HVTN703_DVE_results_by_feature_forest_plot_",tolower(.excluded.dose.group),"_excluded.pdf") ), width = 5.5, height = 6)
...
save( dve_results, file = file.path( tablesOutDir, "HVTN703_DVE_results_by_feature_and_dose_group.Rda" )) 
